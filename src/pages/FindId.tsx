...
export default function FindId() {
  ...
  const sendCode = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/auth/find-id/send-code`, {
        method: "POST",
  ...
  const verifyCode = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/auth/find-id/verify`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setVerifyError(err.message || "문제가 발생했습니다.");
    } finally {
      setVerifyBusy(false);
    }
  };
        ...
        <div className="relative p-10 transition-all duration-300 bg-white border shadow-2xl border-gray-100/50 rounded-[2rem] hover:shadow-3xl backdrop-blur-sm">
          ...
          <div className="mb-8 text-center">
            ...
            <p className="text-sm text-gray-600">
              ...
              {step === "verify" && "이메일로 전송된 인증코드를 입력하세요"}
          ...
          {step === "verify" && (
            <form onSubmit={verifyCode} className="space-y-6">
              ...
              <div className="flex gap-3">
                ...
                <button
                  type="submit"
                  disabled={code.length !== 6 || verifyBusy}
          ...
          {step === "result" && (
            <div className="space-y-6 text-center">
              <div className="p-6 border-2 border-emerald-200 shadow-sm bg-emerald-50 rounded-2xl">
                ...
                <h3 className="mb-2 text-lg font-bold text-emerald-700">아이디를 찾았습니다!</h3>
                <p className="mb-4 text-sm text-emerald-600">회원님의 아이디는 다음과 같습니다.</p>
                <div className="p-4 font-mono text-xl font-bold text-gray-800 bg-white border-2 border-emerald-300 rounded-xl">
                  {foundUserId}
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 relative px-6 py-4 overflow-hidden font-bold text-white transition-all duration-300 shadow-xl bg-gradient-to-r from-emerald-500 via-green-500 to-teal-600 rounded-2xl hover:shadow-2xl hover:scale-[1.02] focus:ring-4 focus:ring-emerald-300 group"
                >
                  <span className="relative flex items-center justify-center gap-2 text-base">
                    로그인하기
